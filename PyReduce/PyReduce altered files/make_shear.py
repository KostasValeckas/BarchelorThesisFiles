...
class Curvature:
    ...
    def _determine_curvature_single_line(self, original, peak, ycen, ycen_int, xwd):
        ...
        #'''
        res = least_squares(
            model, x0=x0, method="trf", loss="soft_l1", f_scale=0.1
        )
        for i in range(len(res.fun)):
            try:
                int(res.fun[i])
            except:
                #print('REEEEEEEEEEEEEE', str(i))
                try:
                    res.fun[i]=np.min(res.fun[i-1],res.fun[i+1])
                except:
                    res.fun[i]=0
        #'''
        if self.curv_degree == 1:
        ...
        if self.plot >= 2:
            #print('Shape of image is %s' %str(img.shape))
            #print('Shape of resfun is %s' %str(res.fun.shape))
