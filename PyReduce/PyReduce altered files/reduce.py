...
class OrderTracing(CalibrationStep):
    ...
    def run(self, files, mask, bias):
        ...
        #print('This is a test, checking for files ----------------')
        #print(files)
        #print(len(files))
        #print(files[0])
        #print(files[1])
        ...
        if len(files)==1:
            ...
            print('There seems to be one order definition file, that is okay.')
        elif len(files)==2:
            print('I believe this is an XSHOOTER order definition file, doing this properly now')
            order_img1, ohead1 = self.calibrate([files[0]], mask, bias, None)
            order_img2, ohead2 = self.calibrate([files[1]], mask, bias, None)
            #sum_order_img1 = np.sum(order_img1)
            #sum_order_img2 = np.sum(order_img2)
            order_img = abs(order_img2 - order_img1)
            plt.imshow(order_img)
            plt.title('True Order Master')
            plt.xlabel("x [pixel]")
            plt.ylabel("y [pixel]")
            bot, top = np.percentile(order_img[order_img != 0], (10, 90))
            plt.imshow(order_img, vmin=bot, vmax=top, origin="lower")
            plt.show()
...
class WavelengthCalibrationMaster(CalibrationStep, ExtractionStep):
    ...
    def run(self, files, orders, mask, curvature, bias, norm_flat):
        ...
        if len(files)==1:
            orig, thead = self.calibrate(files, mask, bias, norm_flat)
            print('There seems to be one slit curvature file, that is okay.')
        elif len(files)==2:
            print('I believe this is an XSHOOTER wavelength, doing this properly now')
            order_img1, thead1 = self.calibrate([files[0]], mask, bias, norm_flat)
            order_img2, thead2 = self.calibrate([files[1]], mask, bias, norm_flat)
            #sum_order_img1 = np.sum(order_img1)
            #sum_order_img2 = np.sum(order_img2)
            orig = abs(order_img2 - order_img1)
...
class SlitCurvatureDetermination(CalibrationStep, ExtractionStep):
    ...
    def run(self, files, orders, mask, bias):
        ...
        if len(files)==1:
            ...
            print('I believe this is an XSHOOTER slit curvature file, doing this properly now')
            order_img1, ohead1 = self.calibrate([files[0]], mask, bias, None)
            order_img2, ohead2 = self.calibrate([files[1]], mask, bias, None)
            #sum_order_img1 = np.sum(order_img1)
            #sum_order_img2 = np.sum(order_img2)
