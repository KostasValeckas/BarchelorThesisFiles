...
# this is the script used to produce a smaller version of the full scale plot for the thesis
# it is only provided for refference. For the full scale plot, see the ATcompared_var.py script
...
fig,(ax1,ax3)=plt.subplots(2,1,figsize=(10,15))
...
#fig.supxlabel("Observed Wavelength (Å)", y=0.05 ,fontsize=14)
#fig.supylabel("Flux-error (erg/s/cm²/Å))",fontsize=14)

for ax in (ax1,ax3):
	ax.tick_params(length=10, width=4,labelsize=20)
	for i in ax.legend(fontsize=15).get_lines(): i.set_linewidth(10)
	ax.yaxis.get_offset_text().set_fontsize(20)


plt.savefig("./ATcompared_var_thesisSnippet.png",bbox_inches="tight",facecolor="white")
plt.show()
